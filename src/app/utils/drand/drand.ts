...
import HttpCachingChainClient from './httpCachingChainClient'
...
class Drand {
    ...
    client: HttpCachingChainClient
    roundNumber: number

    private constructor(roundNumber: number, network: 'testnet' | 'mainnet' = 'testnet') {
        this.roundNumber = roundNumber
        if (network === 'testnet') {
            this.client = new HttpCachingChainClient(new HttpCachingChain(this.DEFAULT_TESTNET_URL))
        } else {
            this.client = new HttpCachingChainClient(new HttpCachingChain(this.DEFAULT_MAINNET_URL))
        }
    }
